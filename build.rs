...
"#,
    ...
    // Rate-limit / revocation safety. Spamhaus DROP is rate-limited (~1/day) and revocable;
    // the non-fatal fetch above means a blocked fetch yields ZERO ranges, which would silently
    // disable IP blocking (is_bad_ip -> false for everything). Surface that loudly, and in
    // strict mode fail the build so production never *silently* ships with IP blocking off.
    if include_ip {
        println!("cargo:rerun-if-env-changed=SPIDER_FIREWALL_IP_STRICT");
        if ip_ranges_v4.is_empty() {
            let msg = "spider_firewall: `ip` feature is enabled but the Spamhaus DROP fetch \
            ...
            println!(
                "cargo:warning=spider_firewall: embedded {} Spamhaus DROP IPv4 range(s)",
                ip_ranges_v4.len()
            );
        }
    }
